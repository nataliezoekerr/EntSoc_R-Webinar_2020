...
**Learning R is not learning statistics!** R can implement any number of statistical tests and analyses, and regardless of if they're appropriate, it will run them on data formatted appropriately. Be careful to have a plan for what statistics you want to calculate *before* you start coding.
...
RStudio makes it easier to navigate between code, the console and output, while also the characteristics of your R environment. You can think of RStudio as an "R organizer".
...
RMarkdown and and RNotebooks allow you to create sharable documents that combine a written narrative, code and results. RMarkdown syntax syntax, which RNotebooks use, can be knit into pdfs or html files. (This was written via RMarkdown). RMarkdown and RNotebooks also allow you to run and view chunks of code within the document.
...
#The arguments for c are are the names of objects to combine
...
mean(nums)
```
Since our object "seuss" contained numbers and words (words are "strings of characters" to your computer), this was formatted as a character class. A vector of 1-100 cooperate with the integer class, so the nums object is an integer class. This reliable behavior is built into the function c()
