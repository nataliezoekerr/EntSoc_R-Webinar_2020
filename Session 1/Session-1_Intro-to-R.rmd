...
```{r loops and if/then statements, include=T}
#A for() loop alone
for(x in 1:10){
  y=2*x
  print(c(x, y))
...
ggplot(data=data, aes(x=column, y=column, color=column))+geom_bar(stat="identity")
...
To take this one step further, we could determine our own bar colors. Typically, this process will be streamlined with a package containing available palettes to choose from. RColorBrewer
is widely-used. We will install this and a younger package called MaizePal. MaizePal is not yet available on CRAN, so we will install it via github using a package called devtools.
...
monarch.map <- world.map +
  geom_point(data=subset(US.iNat.dat), aes(x=longitude, y=latitude, color=Month)) +
...
```
```{r render and save moving map, include=F, warnings=F}
...
![Our monarch gif](Output/monarch.gif)
