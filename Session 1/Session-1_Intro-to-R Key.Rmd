...
reprepsum=function(input, n1, n2){
  nums4 <- rep(input, n1)
  nums5 <- rep(nums4, n2)
  sum(nums5)
}
...
3. Calculate the mean survivors and stdev for each treatment

```{r practice exercise 4, include=T}
Replicate <- rep(x=1:3, each=60)
Treatment <- rep(c("treated", "untreated"), each=30, times=3)
n <- rep(30, times=180)
exer.dat <- data.frame(Replicate, Treatment, n)
exer.dat$Survivors=NA
exer.dat[exer.dat$Treatment=="treated",]$Survivors <- round(rnorm(n=90, mean=10, sd=4))
exer.dat[exer.dat$Treatment=="untreated",]$Survivors <- round(rnorm(n=90, mean=20, sd=4))

exer.dat

exer.dat$Dead <- exer.dat$n-exer.dat$Survivors

exer.stat <- exer.dat %>%
  select(Treatment, Survivors) %>%
  group_by(Treatment) %>%
  summarize(mn=mean(Survivors), sd=sd(Survivors))
```
...
ggplot(data=data, aes(x=column, y=column, fill=column))+geom_col()
...
**Practice exercise 5:**
Create a ggplot2 boxplot displaying our InsectSprays count data.
...
dat$spray <- factor(dat$spray, levels=levels(dat$spray)[c(6,1:5)])

spray.plt5 <- ggplot(data=dat, aes(x=spray, y=count, fill=spray))+
  #"jitter" moves points slightly so we can see points that overlap
  geom_violin()+geom_point(position="jitter", color="grey")+
  theme+theme+ylab("Insects")+xlab("Spray")+scale_fill_manual(values=palette1)+
