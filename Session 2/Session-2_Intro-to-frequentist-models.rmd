...
```{r Practice 5 - Testing assumptions of linear regression, include = T }
...
where the log of mean $\mu_{y}$ is a linear function of *x*. 
...
![Figure 2: Trap color of Venus flytraps from four populations (or eight sub-sites) across their range](Data\Phenotypic-site-differences.jpg) 
...
![Figure 3. Trap color of Venus flytraps from four populations (or eight sub-sites) across their range](Data\VFT_TrapColor_Sites.jpg) 
...
```{r Practice 6 - Probability of infection, include = T }
...
In more complex terms, the Poisson distribution similar to like the binomial distribution in that it has a large "unknown" number of trials *N* and a small probability of an event $p = \lambda / N$, where $\lambda$ is the *k* discrete events (or mean count estimate). The probability of given number of events can be thought of as the probability of counting an individual that occurs over a fixed interval of time or space.
...
coef(wv10b)
exp(coef(wv10b)[2]) # back-transformed slope is the population growth rate

wolves$num.wolves[13] # number of wolves in 1994
exp(coef(wv10b)[2]) * wolves$num.wolves[14] # estimated number of wolves in 1995, N(t+x) = N(t) * r^x
exp(coef(wv10b)[2])^4 * wolves$num.wolves[14]
...
```{r Practice 7 - Poisson distribution, include = T }
...
Here, we will cover how to deal with three common issues that you may encounter with Poisson-distributed data: 
  ...
  2. When count data are over- or under-dispersed for the Poisson distribution
...
ICtab(nf1, nf1_offset, nf2)

nf2 <- glm(Count/hours ~ 1, family = gaussian, data = flickers)
...
wolves$num.wolves[1:30] * exp(coef(wv30c))   
...
```{r Example 8 - Poisson offsets, include = T }
...
> pscl::zeroinfl(formula = y ~ x_count | x_zero, dist = "negbin")
...
The glmmTMB package also allows for hurdle models for count data using the "truncated" families (i.e., `family = truncated_poisson`). Note that use of the Poisson family would indicate a zero-inflated model. Here, we are using the owl dataset that is included in `glmmTMB` package. 
