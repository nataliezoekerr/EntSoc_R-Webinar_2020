...
summary(effects.par)$coefficients # same model as dens2, formula assumes "~ 1 +" 
...
means.par <- lm(mean.density ~ -1 + landuse, data = hares) # removes dummy variable
summary(means.par)$coefficients 
...
gh1 <- lm(LocMax ~ Origin*Treatment, data = hoppers)
...
hares$yearPost <- hares$year - 1991 # re-scale variable, so that it can converge on a MLE
hares.arable <- subset(hares, landuse == "arable")

dens6 <- lm(mean.density ~ yearPost, data = hares.arable)
summary(dens6)

pred <- coef(dens6)[1] + coef(dens6)[2]*1:17
with(subset(hares, landuse == "arable"), plot(yearPost, mean.density, xlab = "Years since beginning of experiment"))
points(1:17, pred, type = "l")
...
dens10 <- lm(log(mean.density) ~ year, data = hares.arable)
...
dens11 <- glm(mean.density ~ year, family = gaussian(link = "log"), data = hares) # glm() instead of lm()
...
dens7 <- lm(mean.density ~ yearPost, data = hares.arable)
dens10 <- lm(log(mean.density) ~ yearPost, data = hares.arable)
dens11 <- glm(mean.density ~ yearPost, family = gaussian(link = "log"), data = hares)
dens12 <- glm(mean.density ~ yearPost, family = Gamma(link = "log"), data = hares)
...
For nested models, we may want to use marginal null hypothesis testing of a saturated model to determine significant effects. For example, we might want to evaluate whether Taylor Checkerspot caterpillars reared on two host plant (i.e., Indian paintbrush and English Plantain) that were either sprayed or not sprayed with herbicide treatment affected their mass.  
...
For our first example, we will be using data from a laboratory diapause experiment to estimate the probability of entering diapause as a function of photoperiod. I put eggs from the mustard white butterfly (*Pieris oleracea*) in five photoperiod treatments, then measured at the individual-level whether they survived (1 for survival, 0 for died) and whether they diapaused (e.g., 1 for entering diapause, 0 for eclosing) conditioned on surviving. These data were used in [Kerr et al. 2020](https://onlinelibrary.wiley.com/doi/abs/10.1111/gcb.14959).  
...
Here, we will be exploring running log-link GLMs using the Poisson error distribution. *explain more*
...
A common issue in ecology is when each observation of count data are not always equally in represented. For example, you may find yourself in a situation where each observation is collected over different lengths of time or you count a number out of a total number.  *Explain* Poisson offset....
...
Here, we will use the negative binomial distribution, which is the most common approach for overdispersion. A negative binomial distribution is the average of different Poisson distributions with different means. Unlike the Poisson where the mean is equal to the variance, the variance for the negative binomial .... *explain*
...
ICtab(nf2, nf3) # variance is linear parameterized
...
Below, we will run all four combinations of random-coefficient models for the brown hares dataset using the log mean density of brown hares as a function of rescaled year ("yearPost") using `lme4::glmer()`.

First, we will run a random-intercept model (probably the most commonly used random-coefficient model) that allows the model estimate for the intercept to come from a second distribution that estimates a mean and variance from intercepts from each region. 
...
for(i in 1:length(unique(hares$region))){
  re_list1[[i]] <- data.frame(region = rep(unique(hares$region)[i], 17), 
                              yearPost = year.vals, 
                              int = rep(ranef(dens_re1)$region[i,1], 17), 
                              slope = rep(fixef(dens_re1)[2], 17))
}
re_dat1 <- do.call("rbind", re_list1)
re_dat1$pred <- re_dat1$int + re_dat1$slope*re_dat1$yearPost # y = B0 + B1x
...
Second, we will run a random-slope model (note that this is often not a sensible model) that allows the model estimate for the slope only to come from a second distribution that estimates a mean and variance from slopes from each region. 
...
re_list2 <- list()
for(i in 1:length(unique(hares$region))){
  re_list2[[i]] <- data.frame(region = rep(unique(hares$region)[i], 17), 
                              yearPost = year.vals, 
                              int = rep(fixef(dens_re2)[1], 17), 
                              slope = rep(ranef(dens_re2)$region[i,1], 17))
}
...
Third, we will run a random-intercept and -slope model that allows the model estimates for both intercept and slopes to come from another distribution that estimates a mean and variance from intercepts and slopes from each region. 
...
re_list3 <- list()
for(i in 1:length(unique(hares$region))){
  re_list3[[i]] <- data.frame(region = rep(unique(hares$region)[i], 17), 
                              yearPost = year.vals, 
...
Finally, we will run a random-intercept and -slope model with a correlation between the coefficients that allows the model estimates for both intercept and slopes to come from another distribution that estimates a mean and variance from intercepts and slopes from each region. 
...
**Practice example 11:** Using the built-in dataset on grouse ticks from lme4 (see ?grouseticks), we want you to evaluate the effects of height above sea level (using "cHEIGHT") on the number of ticks of Red Grouse chicks using random-coefficient models for brood number. We expect you to evaluate the correct error distribution for these data, and then find the winning model using model selection.   

> attach(grouseticks)

```{r Practice 11 - Random-coefficient models }

gt1 <- glmmTMB(TICKS ~ cHEIGHT, family = poisson, data = grouseticks)
...
**Practice example 12:** Using the built-in dataset on grouse ticks, we want repeat practice example 11 except include nested random effects of brood and chick number. We expect you to evaluate the correct error distribution for these data, and then find the winning model using model selection.   
...
plot(x, dnorm(x = x, mean = mu, sd = sd), type = "l", ylab = "Probability", xlab = "y value")
...
p <- 0.8 # Probability of being counted (seen)
sample <- rbinom(n = n, size = N, prob = p)
head(sample)

hist(sample, col = "grey")

dbinom(x = 30, size = N, prob = p) # you cannot have 30 dead
